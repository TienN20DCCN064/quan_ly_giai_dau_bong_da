document.addEventListener("DOMContentLoaded", function() {
  ...
  loginForm.addEventListener("submit", async function(e) {
    ...
    try {
      ...
      const res = await hamChung.dangNhap(formData);
      ...
      if (res.token) {
        ...
        if(res.user.vai_tro === "Quản lý đội bóng"){
          ...
          const data = await hamChung.layDoiBongTheoQL(res.user.tai_khoan);
          console.log(data);
          //load danh sách đội bóng

          if (data.length === 1) {
            const maDoi = data[0].ma_doi_bong;
            window.location.href = `/frontend/view/quanly/doi_bong/cau_thu.html?ma_doi_bong=${maDoi}`;
            return;
          }
          ...
          data.forEach(doi => {
            ...
            item.onclick = () => {
              window.location.href = `/frontend/view/quanly/doi_bong/cau_thu.html?ma_doi_bong=${doi.ma_doi_bong}`;
            };
